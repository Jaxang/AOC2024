...
def star1(txt):
    games = parse_input(txt)
    sum=0
    for game in games:
        tmp=solve_matrix(game)
        #print(tmp)
        sum+=tmp
    return sum
...
def star2(txt):
    ...
    for game in games:
        ...
        tmp=solve_matrix(((ax, ay), (bx, by), (px+10000000000000,py+10000000000000)))
        #print(tmp)
        sum+=tmp
    return sum
    
def solve_matrix(game):
    #print(game)
    (ax, ay), (bx, by), (px,py) = game
    det = ax*by-bx*ay
    a = int((by*px-bx*py)/det)
    b= int((-ay*px+ax*py)/det)
    #print(a*ax+b*bx, px)
    if a*ax+b*bx==px and a>=0 and b>=0:
        #print("a")
        return a*3+b
    return 0

def parse_input(txt):
    return list(map(parse_game, txt.split("\n\n")))
...
def parse_game(txt):
    ...
    bx, by = b.split(" ")[-2:]
    px, py = p.split(" ")[-2:]
    return list(map(lambda xy: (int(xy[0][2:]), int(xy[1][2:])), [(ax.strip(","),ay), (bx.strip(","),by), (px.strip(","),py)]))
